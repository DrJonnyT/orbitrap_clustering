...
#ds_dataset_cat is the categorical data series of which dataset there is
#suptitle is the title to go at the top of the plot
def plot_cluster_heatmap_BeijingDelhi(c,ds_dataset_cat,suptitle,ylabel,integer_labels=False):
    fig,ax = plt.subplots(2,2,figsize=(9,9))
    ax=ax.ravel()
    ...
    for idx in range(4):
        ...
        c_thiscat = c[ds_dataset_cat == cat]
        ds_dataset_cat_thiscat = ds_dataset_cat[ds_dataset_cat == cat]
    
        xbins = pd.date_range(ds_dataset_cat_thiscat.index.floor("1D").min(),ds_dataset_cat_thiscat.index.ceil("1D").max(),freq="1D") 
        ybins = np.unique(c)
        #pdb.set_trace()
        #ax[idx].yaxis.set_major_locator(MaxNLocator(integer=True))
        ybins= np.append(ybins,ybins.max() + 1) -0.5
        ax[idx].hist2d(ds_dataset_cat_thiscat.index,c_thiscat, bins=[xbins,ybins])
        ...
        #Move y axis labels slightly
        #ax[idx].set_yticks(np.unique(c_thiscat) + 0.5)
        #ax[idx].yaxis.set_label_coords(0,0.5)
